...
{
	int allocationSize = get_allocation_size(cgm->numberCount);					// malloc kullanarak host memory alani, alloc yapilmis olsun..
	cudaError_t resultHost = cudaHostRegister(cgm->cpu_p, allocationSize, 0);	// cudaHostRegister ile yalnizca cudaMemcpy yapicagimiz zaman bellegi sabitleyebiliriz(pinleriz) ..	
	assert(resultHost == cudaSuccess);

	cudaError_t result = cudaMemcpy(cgm->gpu_p, cgm->cpu_p, get_allocation_size(cgm->numberCount), cudaMemcpyHostToDevice);
	assert(result == cudaSuccess);

	cudaError_t resultUn = cudaHostUnregister(cgm->cpu_p);						// pin lenmis bellek bolgesi copy sonrasi, diger processlerin kullanabilmesi icin serbest birakilir.
	assert(resultUn == cudaSuccess);
