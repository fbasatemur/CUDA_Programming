...
{
	const int pinMemoryLimit = 100 * 1024 * 1024;					// 100 MB uzeri sabitlenmis(pin lenmis) bellek uzerinden kopyalama yapsin
	int allocationSize = get_allocation_size(cgm->numberCount);
	
	cudaError_t result;

	if (allocationSize > pinMemoryLimit)
	{
		result = cudaHostRegister(cgm->cpu_p, allocationSize, 0);	// cudaHostRegister ile yalnizca cudaMemcpy yapicagimiz zaman bellegi sabitleyebiliriz(pinleriz) ..	
		assert(result == cudaSuccess);
	}
	...
	if (allocationSize > pinMemoryLimit)
	{
		result = cudaHostUnregister(cgm->cpu_p);					// pin lenmis bellek bolgesi copy sonrasi, diger processlerin kullanabilmesi icin serbest birakilir.
		assert(result == cudaSuccess);
